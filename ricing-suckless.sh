...
#
# base contains: bash bzip2 coreutils cryptsetup device-mapper dhcpcd diffutils e2fsprogs
#  file filesystem findutils gawk gcc-libs gettext glibc grep gzip inetutils iproute2
#  iputils jfsutils less licenses linux logrotate lvm2 man-db man-pages mdadm nano netctl
#  pacman pciutils pcmciautils perl procps-ng psmisc reiserfsprogs s-nail s-nail sed shadow
#  sysfsutils systemd-sysvcompat systemd-sysvcompat tar texinfo usbutils util-linux util-linux
#  vi which xfsprogs
#
# base-devel contains: autoconf automake binutils bison fakeroot file findutils flex gawk gcc gettext
#  grep groff gzip libtool m4 make pacman patch pkg-config sed sudo systemd texinfo util-linux which
#
...
# 
pacstrap -i /mnt base base-devel wireless_tools networkmanager xf86-video-intel vim wget 
...
# imagemagick	  : an image viewing/manipulation program, screenshots through import
...
#
pacman --needed -Sy xorg-xinit xorg-server ttf-inconsolata rsync openvpn openresolv git unzip unrar p7zip ntfs-3g exfat-utils dosfstools ranger w3m htop feh ffmpeg mpv calcurse newsboat pulseaudio pulseaudio-alsa arandr pavucontrol pamixer mediainfo poppler highlight mpd mpc imagemagick atool libcaca compton transset-df mupdf evince youtube-dl youtube-viewer pandoc python-dbus python-gobject blender gimp transmission-cli screenfetch

## functions that will be needed for st, dwm, surf and lemonbar
#
# aurinstall is lukesmithxyz creation. many thanks, sir.
# aurget is my alteration, because sometimees you're going to want to alter the config.h before compiling.
#         + don't forget to updpkgsums after changing anything. other wise, makepkg will complain.
#
aurinstall() { curl -O https://aur.archlinux.org/cgit/aur.git/snapshot/$1.tar.gz && tar -xvf $1.tar.gz && cd $1 && makepkg --noconfirm -si && cd .. && rm -rf $1 $1.tar.gz ;}
aurget() { curl -O https://aur.archlinux.org/cgit/aur.git/snapshot/$1.tar.gz && tar -xvf $1.tar.gz && cd $1 ;}
